...
class DeviceInfo(BaseModel):
    ...
    @cached_property
    def unique_id(self, use_hash: bool = False) -> str:
        if use_hash:
...
class Device:
    ...
    def __init__(
        ...
        if self._use_status:
            ...
            self._mqtt_client.add_callback(f"{discovery_prefix}/status", self._ha_status_callback)
    ...
    def _component_init(self, key_id: str, component: ComponentBase) -> None:
        ...
        if 'qos' in self._discovery_info.model_fields_set and 'qos' not in component.model_fields_set:
            component.qos = self._discovery_info.qos
            component.__pydantic_fields_set__.remove('qos')
        if 'encoding' in self._discovery_info.model_fields_set and 'encoding' not in component.model_fields_set:
        ...
        if component.base_topic is Unset:
            ...
            topics = filter(lambda f: f.endswith('_topic') or f == 'topic', component.model_fields_set)
            for topic in topics:
        ...
        if isinstance(component, BareEntityBase) and component.unique_id is Unset:
            sha256 = hashlib.sha256(component.model_dump_json(exclude_unset=True, by_alias=True).encode('utf-8'))
    ...
    def component_register(self, name: str, component: ComponentBase, announce: bool = True) -> None:
