...
class Event(StatefulComponent, EntityBase):
    ...
    def trigger(self, type: str, attr: dict[str, Any] | None = None) -> None:
...
class Image(StatefulComponent, EntityBase):
    ...
    def _check_content_type_mutex(self) -> Self:
        ...
        if 'content_type' not in self.model_fields_set and 'url_topic' not in self.model_fields_set:
            raise ValueError('One of `url_topic` or `content_type` must be set')
        return self
    ...
    def set_state(
        ...
        if color_mode is not None:
            ...
            if color is not None:
                data['color'] = dict()  # Linter weirdness
                match color_mode:
                    case LightColorMode.HS:
                        data['color']['h'] = color[0]
                        data['color']['s'] = color[1]
                    case LightColorMode.XY:
                        data['color']['x'] = color[0]
                        data['color']['y'] = color[1]
                    ...
                    case LightColorMode.RGBWW:
                        ...
                        data['color']['g'] = color[1]
                        data['color']['b'] = color[2]
                        data['color']['c'] = color[3]
                        data['color']['w'] = color[4]
        if brightness is not None:
            data['brightness'] = min(max(brightness, 0), self.brightness_scale)
        if color_temp is not None:
            if self.color_temp_kelvin:
                data['color_temp'] = min(max(color_temp, self.min_kelvin), self.max_kelvin)
            else:
